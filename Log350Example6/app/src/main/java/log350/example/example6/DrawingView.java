...
public class DrawingView extends View {
    ...
    static final int MODE_EFFACER = 4; // the user is drawing a lasso to select shapes
    static final int MODE_CREER = 5; // the user is drawing a lasso to select shapes
    ...
    MyButton effacerButton = new MyButton("Effacer", 10, 240, 140, 140);

    MyButton creerButton = new MyButton("Creer", 10, 400, 140, 140);
                ...
                public boolean onTouch(View v, MotionEvent event) {
                    ...
                    switch (currentMode) {
                        ...
                        case MODE_LASSO:
                            ...
                            break;
                        case MODE_EFFACER:
                            shapeContainer.shapes.removeAll(selectedShapes);
                            selectedShapes.clear();
                        ...
                        case MODE_CREER:
                            if (type == MotionEvent.ACTION_DOWN) {
                                ...
                                if (cursorContainer.getNumCursors() > 0) {
                                    ArrayList<Point2D> points = new ArrayList<Point2D>();
                                    int count = cursorContainer.getNumCursors();
                                    for (int i = 1; i < count; i++) {
                                        points.add(cursorContainer.getCursorByIndex(i).getCurrentPosition());
                                    }
                                    shapeContainer.addShape(Point2DUtil.computeConvexHull(points));
                                    while (cursorContainer.getNumCursors() > 0) {
                                        cursorContainer.removeCursorByIndex(0);
                                    }
                                }
                                if (cursorContainer.getNumCursors() == 0) {
                                    currentMode = MODE_NEUTRAL;
